...
static void parse_leg_call_stats(struct call_stats *codec, bencode_item_t *, struct sip_msg *);
...
static int call_stats_parse(struct call_stats *stats) {
	if (pv_parse_var(&stats->tag_param, &stats->tag_pv, &stats->got_tag_pv))
		return -1;
	if (pv_parse_var(&stats->media_ip_param, &stats->media_ip_pv, &stats->got_stats_pv))
		return -1;
	if (pv_parse_var(&stats->codec_param, &stats->codec_pv, &stats->got_stats_pv))
		return -1;
...
static void avp_print_call_stats(pv_elem_t *elem, bencode_item_t *dict, char *key, struct sip_msg *msg) {
	str buffer_str;
...
static void parse_leg_call_stats(struct call_stats *stats, bencode_item_t *dict, struct sip_msg *msg)
{
	str tag_str;
	char tag_string[256] = "";
	...
	streams = bencode_dictionary_get_expect(medias->child, "streams", BENCODE_LIST);
	...
	if (stats->media_ip_pv) {
		endpoint = bencode_dictionary_get_expect(streams->child, "endpoint", BENCODE_DICTIONARY);
